    ...
    function control($tipo_x=C_MODO_DOC){
		if ($tipo_x==C_MODO_DOC){	
			...
			if($this->onclick_value!=""){
				...
				$aux .= "\n}// end function ";
				//$aux = "add_eventos(document,{\"onclick\":\"$this->onclick_value\"})";
				
				$this->js_post($aux,C_MODO_AGREGAR);
			}// end if
			if($this->onload_value!=""){
				$aux .= "window.onload = function (){";
				$aux .= $this->onload_value;
				$aux .= "\n}// end function ";
				$this->js_post($aux,C_MODO_AGREGAR);
			}// end if
